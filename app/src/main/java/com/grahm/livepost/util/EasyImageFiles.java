...
class EasyImageFiles {
    ...
    public static String getFolderName(Context context) {
        return PreferenceManager.getDefaultSharedPreferences(context).getString(pl.aprilapps.easyphotopicker.BundleKeys.FOLDER_NAME, DEFAULT_FOLDER_NAME);
    }

    public static File tempImageDirectory(Context context) {
        boolean publicTemp = PreferenceManager.getDefaultSharedPreferences(context).getBoolean(pl.aprilapps.easyphotopicker.BundleKeys.PUBLIC_TEMP, false);
...
//    }
    ...
    public static File getCameraVideosLocation(Context context) throws IOException {
        ...
        File dir = new File(cacheDir, DEFAULT_FOLDER_NAME);
        if (!dir.exists()) dir.mkdirs();
        File videoFile = File.createTempFile(UUID.randomUUID().toString(), ".mp4", dir);
        return videoFile;
    }
