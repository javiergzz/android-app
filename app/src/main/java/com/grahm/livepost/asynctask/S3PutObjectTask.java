...
public class S3PutObjectTask extends AsyncTask<Uri, String, String> {
    ...
    protected void onPreExecute() {
        ...
        dialog.setMessage("Compressing photo...");
    ...
    protected void onPostExecute(String result) {
        ...
        if(mListener != null){
            String picture_url = "https://s3.amazonaws.com/livepostrocks/profile/" + mPictureName + ".jpg";
            mListener.onSuccess(picture_url);
        }
    }
    ...
    private String uploadImage(String pictureName,Uri srcUri,int mDstWidth, int mDstHeight){
        ...
        try {
            PutObjectRequest por = new PutObjectRequest( Config.PROFILE_PICTURE_BUCKET, pictureName, bs, metadata).withCannedAcl(CannedAccessControlList.PublicRead);
            mS3Client.putObject(por);
            ResponseHeaderOverrides override = new ResponseHeaderOverrides();
            override.setContentType("image/jpeg");
            GeneratePresignedUrlRequest urlRequest = new GeneratePresignedUrlRequest( Config.PROFILE_PICTURE_BUCKET, pictureName );
